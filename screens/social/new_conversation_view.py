...
class NewConversationView(Screen):
            ...
            with VerticalScroll(id="main_box"):
                ...
                with VerticalScroll(id="friends_list_container"):
                    if friends:
                        ...
                        yield Static(
                            "Você ainda não possui amigos. Adicione amigos na tela de Amigos!",
    ...
    def _build_friend_row(self, friend) -> Horizontal:
        ...
        - nome completo e username do amigo;
        - botão verde para iniciar a conversa.
        """

        username_display = (
            f"@{friend.username}" if friend.username else friend.email
        )

        info_column = Vertical(
            Static(friend.name, classes="friend_name"),
            Static(username_display, classes="friend_username"),
            classes="friend_info",
        )

        start_button = Button(
            "💬 Iniciar conversa",
            id=f"start_{friend.user_id}",
            classes="start_chat_button",
            variant="success",
        )
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "home_button":
            ...
            friends = friendship_services.list_friends(self.user_id)
            partner_name = next(
                (f.name for f in friends if f.user_id == partner_id),
